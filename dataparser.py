...
class DataParser(Dataset):
    ...
    def __init__(self, sequence, root_dir="../kitti_datasets"):
        ...
        self.dx = torch.from_numpy(self.dx).view(
            self.dx.shape[0], -1).type(torch.FloatTensor).cuda()
        self.dz = torch.from_numpy(self.dz).view(
            self.dx.shape[0], -1).type(torch.FloatTensor).cuda()
        self.dth = torch.from_numpy(self.dth).view(
            self.dx.shape[0], -1).type(torch.FloatTensor).cuda()
        ...
        self.im_np = torch.from_numpy(
            self.im_np).type(torch.FloatTensor).cuda()
    ...
    def __getitem__(self, idx):
        ...
        data = {"img_l1": img_l1, "img_l2": img_l2, "x": x, "z": z,
                "dx": dx,  "dz": dz, "dth": dth, "dt": dt, "t": time}
